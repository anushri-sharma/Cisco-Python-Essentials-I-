...
* The user is asked to enter a number using `input()`.
* If the number is not equal to `secret_number`, the loop continues.
* If the correct number is entered, the program prints the success message and uses `break` to stop the loop.
